...
variable "http_listeners" {
  ...
  # http_listeners = [{ name = "", frontend_ip_configuration = "", port = "", protocol = "", host_name = "", host_names = "hostname1,hostname2", ssl_certificate_name = "", firewall_policy_id = "" }]
}
variable "probes" {
  type        = list(map(string))
  default     = []
...
variable "request_routing_rules" {
  ...
  # request_routing_rules = [{ name = "", http_listener_name = "", backend_address_pool_name = "", backend_http_settings_name = "", priority = 10, rewrite_rule_set_name = "" }]
}
